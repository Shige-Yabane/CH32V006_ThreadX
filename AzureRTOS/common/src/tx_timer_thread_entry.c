...
/* Retained diagnostic for the timer thread's initial yield. */
extern volatile ULONG tx_diag_timer_thread_stage;
...
/* CH32V203 port diagnostic: debugger-read only, no scheduling changes. */
volatile ULONG threadx_timer_thread_entry_count;
volatile ULONG threadx_timer_thread_loop_count;
volatile ULONG threadx_timer_thread_suspend_count;
...
TX_TIMER                    *timer_ptr;
    ...
    if (timer_thread_input == TX_TIMER_ID)
    {
        ...
        tx_diag_timer_thread_stage = 1U;

        /* Yes, valid thread entry, proceed...  */
        threadx_timer_thread_entry_count++;

        /* Now go into an infinite loop to process timer expirations.  */
        while (TX_LOOP_FOREVER)
        {

            tx_diag_timer_thread_stage = 2U;

            threadx_timer_thread_loop_count++;
            ...
            *_tx_timer_current_ptr =  TX_NULL;
            ...
            if (_tx_timer_expired == TX_FALSE)
            {
                ...
                threadx_timer_thread_suspend_count++;
