...
UINT            map_index;
    ...
    /* The CH32V SysTick can occur while the ThreadX scheduler is idle.
       In that state there is deliberately no current thread context.  The
       WCH vector sees the newly set execute pointer and enters the scheduler
       after this function returns, so do not call _tx_thread_system_return()
       with a NULL current_thread here. */
    if ((current_thread != TX_NULL) && (current_thread != _tx_thread_execute_ptr))
    {
